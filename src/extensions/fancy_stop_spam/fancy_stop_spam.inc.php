...
class Fancy_stop_spam {
    ...
    public function print_new_users() {
        ...
        $found_user_datas = array();
        while ($user_data = $forum_db->fetch_assoc($result)) {
            $found_user_datas[] = $user_data;
        }

        $forum_page['item_count'] = 0;

        if (!empty($found_user_datas)) {
            // Make request to StopForumSpam
            $sfs_request_email_data = array("email" => array());
            $sfs_request_ip_data = array("ip" => array());
            foreach ($found_user_datas as $found_user) {
                $sfs_request_email_data["email"][] = $found_user["email"];
                $sfs_request_ip_data["ip"][] = $found_user["registration_ip"];
            }
            ...
            if ($fancy_stop_spam_email_data === FALSE || $fancy_stop_spam_ip_data === FALSE) {
                message("Cannot get info from Stop Forum Spam server. Try again later.");
            }

            $users_data = '';
            foreach ($found_user_datas as $found_user) {
                $username_row = '-';
                if (intval($found_user['id'], 10) > 0 && !empty($found_user['username'])) {
                    if ($found_user['id'] == '1') {
                        $username_row = forum_htmlencode($found_user['username']);
                    } else {
                        $username_row = '<a href="'.forum_link($forum_url['user'], forum_htmlencode(intval($found_user['id'], 10))).'">'.
                                        forum_htmlencode($found_user['username']).
                                    '</a>';
                    }
                }

                $email_status = $this->get_sfs_status_for_email($found_user["email"], $fancy_stop_spam_email_data, $spam_status_email);
                $ip_status = $this->get_sfs_status_for_ip($found_user["registration_ip"], $fancy_stop_spam_ip_data, $spam_status_ip);

                // NUMBER of POSTS
                $num_posts_row = '0';
                if (intval($found_user['num_posts'], 10) > 0) {
                    $num_posts_row = '<a href="'.forum_link($forum_url['search_user_posts'], $found_user['id']).'">'.$found_user['num_posts'].'</a>';
                }

                // NUMBER of LOGS
                $num_logs_row = '0';
                if (intval($found_user['num_logs'], 10) > 0) {
                    $num_logs_row = '<a href="'.forum_link($forum_url['fancy_stop_spam_profile_section'], $found_user['id']).'">'.$found_user['num_logs'].'</a>';
                }
    ...
    private function make_request_to_sfs($data = array()) {
        ...
        if (!empty($data)) {
            ...
            $check_result = get_remote_file($check_url, 15);
