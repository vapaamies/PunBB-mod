...
function createSitemap()
{
  ...
  $array[] = global_link(forum_link($forum_url['index']));
  if ($forum_user['g_view_users'] == '1')
    $array[] = global_link(forum_link($forum_url['users']));
  ...
  while ($cur_topic = $forum_db->fetch_assoc($result))
  {
    ...
    {
      $array[] = global_link(forum_link($forum_url['forum'], array($cur_topic['fid'], sef_friendly($cur_topic['forum_name']))));
      $cur_forum = $cur_topic['fid'];
    }
    $array[] = global_link(forum_link($forum_url['topic'], array($cur_topic['id'], sef_friendly($cur_topic['subject']))));
  }
  ...
  while ($cur_forum = $forum_db->fetch_assoc($result))
    $array[] = global_link(forum_link($forum_url['forum'], array($cur_forum['id'], sef_friendly($cur_forum['forum_name']))));

  $xml = '<?xml version="1.0" encoding="utf-8"?>'."\n".'<!-- Created by Sitemap extension 1.4 for PunBB -->'."\n".'<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">'."\n";
